...
public class LoginTest extends BaseTest {
	...
	@FrameworkAnnotation(author = { AuthorType.Rudra, AuthorType.NISHANT }, category = { CategoryType.BVT,
			CategoryType.REGRESSION })
	@Test(groups = { "BVT",
			"REGRESSION" }, description = "To check whether the User can not login with Invalid Credentials", dataProvider = "getTestData", dataProviderClass = DataProviderUtils.class)
...
//                .clickLogin();
		...
		Assert.assertEquals(errorInvalidCreds, expectedError, "Assertion for Invalid credentials Error message");
	}
	...
	// @Test(description = "To check whether the User can not login with Invalid
	// Credentials")
...
//        String errorInvalidCreds =  Driver.driver.findElement(By.id("spanMessage")).getText();
		// String errorInvalidCreds =
		// Driver.threadLocal.get().findElement(By.id("spanMessage")).getText();
		String errorInvalidCreds = DriverManager.getDriver().findElement(By.id("spanMessage")).getText();

		// Assert.assertEquals(errorInvalidCreds, "Invalid credentials",
//                "Assertion for Invalid credentials Error message");
		Assert.assertEquals(errorInvalidCreds, "æ— æ•ˆçš„å‡­è¯�", "Assertion for Invalid credentials Error message");
